(function(global, document) {
    ...
    function autoExpander(el) {
        return function() {
            el.style.height = "";
            el.style.height =
                Math.min(Math.max(el.scrollHeight, 75), 400) + "px";
        };
    }
    ...
    function makeEvent(cls, key, fn) {
        return function onEvent(event) {
            ...
            if (!(contains(cur, cls) && (id = getData(cur, key)))) {
                return;
            }
    ...
    var _makeCards = function(parentMap, cur) {
        ...
        currentParent.forEach(function(comment) {
            ...
            avatar.innerHTML = comment.name[0];
            body.innerHTML = _showdownConverter.makeHtml(comment.comment);
            subtitle.innerHTML = timeDifference(
                Date.now(),
    ...
    Commento.init = function(configuration) {
        ...
        _commentoCssUrl =
            configuration.commentoCssUrl || _serverUrl + _commentoCssUrl;
        ...
        loadJS(_showdownUrl, function() {
            ...
            // addEvent(name, "keypress", function(e) {
            //     if (e.keyCode === 13) _postRoot();
            // });
            addEvent(
                commento,
                "click",
                makeEvent(SHOW_REPLY_JS, COMMENT_ID_DATA, _showReply)
            );
            addEvent(
                commento,
                "click",
                makeEvent(COLLAPSE_THREAD_JS, COMMENT_ID_DATA, _collapseThread)
            );
            addEvent(
                commento,
                "click",
                makeEvent(EXPAND_THREAD_JS, COMMENT_ID_DATA, _expandThread)
            );
            addEvent(
                commento,
                "click",
                makeEvent(CANCEL_JS, COMMENT_ID_DATA, _cancelReply)
            );
            addEvent(
                commento,
                "click",
                makeEvent(SUBMIT_JS, COMMENT_ID_DATA, _submitReply)
            );
